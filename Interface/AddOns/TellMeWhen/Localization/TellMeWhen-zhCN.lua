...
L["ANN_WHISPERTARGET_DESC"] = "输入你想要密语的玩家名字,仅可密语同服务器/同阵营的玩家.可以使用建议列表中的单位变量."
L["ASCENDING"] = "升序"
L["ASPECT"] = "守护"
L["AURA"] = "光环"
L["BACK_IE"] = "转到上一个"
...
L["CLEU_COMBATLOG_OBJECT_NONE_DESC"] = "勾选以排除WoW客户端完全未知的单位."
...
L["EVENTS_SETTINGS_PASSTHROUGH_DESC"] = [=[允许在触发该事件后去触发另一个事件,如果取消勾选,在该事件触发并输出了文字/音效之后,图标不再处理同时触发的其他任何事件.
...
L["ICONMENU_CUSTOMTEX_DESC"] = "如果你要更改此图标显示的材质,输入你想使用的某个材质的法术名称或ID. 你也可以输入一个材质路径, 例如'Interface/Icons/spell_nature_healingtouch', 假如材质路径为'Interface/Icons'可以只输入'spell_nature_healingtouch'."
...
L["Stunned"] = "被昏迷"
L["SUG_ATBEGINING"] = "建议与提示:匹配开头"
L["SUG_ATBEGINING_DESC"] = [=[如果勾选,建议列表将只显示名称开头位置与正在输入的文字完全相同的法术,否则将显示任意位置有包含此名称的法术.

举例说明:比如你输入了一个'言'字,在勾选的情况下,提示建议列表只会显示'言'字开头的法术,如果没有勾选,它会显示像是'暗言术:痛'这样的法术.]=]
