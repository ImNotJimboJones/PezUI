...
L["ANN_WHISPERTARGET_DESC"] = "輸入你想要密語的玩家名字, 僅可密語同伺服器/同陣營的玩家.可以使用建議列表中的單位變數."
L["ASCENDING"] = "升序"
L["ASPECT"] = "守護"
L["AURA"] = "光環"
L["BACK_IE"] = "轉到上一個"
...
L["CLEU_COMBATLOG_OBJECT_NONE_DESC"] = "勾選以排除WoW客戶端完全未知的單位."
...
L["EVENTS_SETTINGS_PASSTHROUGH_DESC"] = [=[允許在觸發該事件後去觸發另一個事件,如果取消勾選,在該事件觸發並輸出了文字/音效之後,圖示不再處理同時觸發的其他任何事件.
...
用建議列表插入條目時會自動添加在滑鼠提示資訊中顯示的時間(注:自動添加時間功能支持提示資訊中有顯示冷卻時間的法術以及提示資訊中有註明持續時間的大部分法術,如果一個法術同時存在上述兩種時間,會優先選擇添加冷卻時間,假如自動添加的時間不正確,請自行手動更改).]=]
...
此外,該圖示不會顯示已經在另一個整合圖示顯示的任何群組/圖示ID接近1的圖示.在主選項或群組設定中可以更改更新排序.]=]
...
L["ICONMENU_CUSTOMTEX_DESC"] = [=[如果你要更改此圖示顯示的材質, 輸入你想使用的某個材質的法術名稱或ID. 
...
L["SORTBYNONE_META_DESC"] = "如果勾選,被檢查的圖示將使用上面的列表所設定的順序來排列."
...
L["Stunned"] = "擊暈"
L["SUG_ATBEGINING"] = "建議與提示:匹配開頭"
L["SUG_ATBEGINING_DESC"] = [=[如果勾選,建議列表將只顯示名稱開頭位置與正在輸入的文字完全相同的法術,否則將顯示任意位置有包含此名稱的法術.

舉例說明:比如你輸入了一個'言'字,在勾選的情況下,提示建議列表只會顯示'言'字開頭的法術,如果沒有勾選,它會顯示像是'暗言術:痛'這樣的法術.]=]
...
L["UIPANEL_FONT_SIZE_DESC"] = "更改圖示堆疊數量文字的字體大小.如果已使用Masque並且所設定的皮膚的字體大小已定義將忽略此設定值."
