--[[*****************************************************************
	...
	Author: Xinhuan @ US Barthilas Alliance
	*****************************************************************
	Description:
...
local MAX_GUILDBANK_TABS = MAX_GUILDBANK_TABS or 6
...
function BankItems_SlashHandler(msg)
	...
	elseif msg == "" then
		...
		if strfind(msg, "open (.*)") then
			local selfPlayerRealm = strtrim(GetCVar("realmName"))
...
function BankItems_SaveInvItems(bagID)
	...
	do
		...
		if itemLink and count > 0 then
			...
			selfPlayer.Bag100[20] = delTable(selfPlayer.Bag100[20])
		end
	end
	...
	if BankItems_Save.GroupExportData then
						...
						for bagItem = 1, realSize do
							if theBag[bagItem] and type(theBag[bagItem].link) == "string" then
								...
								if strfind(strlower(temp), searchText, 1, true) then
									...
									if bagNum >= 0 and bagNum <= 4 or bagNum == -2 then
		...
		for itemName, whotable in pairs(data) do
			...
			for who, counttable in pairs(whotable) do
				...
				local i, j, k, l = GUILD_BANK, counttable.gbank, CURRENCY, counttable.currency
				local flag = 6
				if not l then
					flag = flag - 1
				end
				...
				if not f then
					flag = flag - 1
					e, f, g, h, i, j = g, h, i, j, k, l
				end
				if not d then
					flag = flag - 1
					c, d, e, f, g, h, i, j = e, f, g, h, i, j, k, l
				end
				if not b then
					flag = flag - 1
					a, b, c, d, e, f, g, h, i, j = c, d, e, f, g, h, i, j, k, l
				end
				line2 = line2 + 1
				if flag == 6 then
					t[line2] = format("     %d %s (%s %d, %s %d, %s %d, %s %d, %s %d, %s %d)", counttable.count, name, a, b, c, d, e, f, g, h, i, j, k, l)
				elseif flag == 5 then
...
function BankItems_AddTooltipData(self, ...)
	...
	if not BankItems_TooltipCache[item] then
		...
		if BankItems_SelfCache[item] then
			...
			local a, b, c, d = L["Bank"], counttable.bank, L["Bags"], counttable.inv
			local e, f, g, h = L["Equipped"], counttable.equipped, MAIL_LABEL, counttable.mail
			local i, j, k, l = AUCTIONS, counttable.auction, CURRENCY, counttable.currency
			local flag = 6
			if not l then
				flag = flag - 1
			end
			...
			if flag == 6 then
				text = format("%s %s %d [%s %d, %s %d, %s %d, %s %d, %s %d, %s %d]", strsplit("|", selfPlayerName), L["has"], counttable.count, a, b, c, d, e, f, g, h, i, j, k, l)
			elseif flag == 5 then
		...
		if BankItems_Cache[item] then
			for who, counttable in pairs(BankItems_Cache[item]) do
				...
				if flag == 6 then
					text = format("%s %s %d [%s %d, %s %d, %s %d, %s %d, %s %d, %s %d]", strsplit("|", who), L["has"], counttable.count, a, b, c, d, e, f, g, h, i, j, k, l)
				elseif flag == 5 then
