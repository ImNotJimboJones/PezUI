...
function MrTrader:OnInitialize()
	...
	self:RegisterBucketEvent({"TRADE_SKILL_UPDATE", "TRAINER_UPDATE"} , 1, "UpdateLinks");
...
function MrTrader:OnLogin()
	self.mapUpdateBucket = self:RegisterBucketEvent("WORLD_MAP_UPDATE", 1, "OnMapUpdate");
end
...
function MrTrader:OnMapUpdate()
	...
	if didUpdate then
		self:UnregisterBucket(self.mapUpdateBucket);
	end
end
...
function MrTrader:PrintTable( table, indent )
	if(indent == nil) then indent = "" end;
	for k, v in pairs(table) do
		if(type(v) == "table") then
			print(indent .. k .. ": {");
			MrTrader:PrintTable(v, indent .. " ");
			print(indent .. "}");
		...
		elseif(type(v) == "boolean") then
			if(v) then print(indent .. k .. ": true") else print(indent .. k .. ": false") end
		else
			print(indent .. k .. ": " .. v);
		end
	end
...
function MrTrader:TablePairPack(...)
	local t,k,v;
	t = {};
	for i = 1, select("#", ...), 2 do
		k = select(i, ...);
