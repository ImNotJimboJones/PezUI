...
function MRTHooks_TradeSkillFrame_Show()
	-- OnDemand load the appropriate module and show it.
...
function MRTHooks_TradeSkillFrame_Update()
	if( not MrTrader:ShouldUseCustomWindow() ) then
		MRTHooks.hooks["TradeSkillFrame_Update"]();
	end
end

function MRTHooks:HookTradeSkillFrameUpdate()
	if( TradeSkillFrame_Update ~= nil and
		TradeSkillFrame_Update ~= MRTHooks_TradeSkillFrame_Update and
		self.hooks["TradeSkillFrame_Update"] == nil ) then
		self:RawHook("TradeSkillFrame_Update", MRTHooks_TradeSkillFrame_Update, true);
	end
end
...
function MRTHooks:TradeSkillFrame_LoadUI()
	...
	self:HookTradeSkillFrameUpdate();
end