...
local LIBRARY_VERSION_MINOR = tonumber(string.match("126.1", "(%d+)") or 1)
...
-- handle zoned areas: these don't get harvested properly. Just symlink them to whichever ID is known at the time (ie, has cont/zone nonzero)
local zone_aliases = {
	-- Gilneas
	[545]=539, 
	[678]=539,
...
local function harvestMapData( HarvestedMapData )
	...
	if ( numFloors > 0 ) then
		for f = 1, numFloors do
			...
			if _ and not TLx then TLx,TLy,BRx,BRy=0,0,3200,1600 end  -- sinus: fix for "uncoorded" maps
			if ( TLx and TLy and BRx and BRy ) then
				mapData[f] = {};
				if not ( TLx < BRx ) then
					TLx = -TLx;
...
aharvestMapData=harvestMapData --sin
...
local function activate( newInstance, oldInstance )
	if ( oldInstance ) then -- this is an upgrade activate
		...
		for id=1,10000 do
			local mid = zone_aliases[id] or id  -- EVIL.
