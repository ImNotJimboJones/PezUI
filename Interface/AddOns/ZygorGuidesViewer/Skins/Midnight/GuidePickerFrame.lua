...
-- 	FR.GuideMacro.cooldown=CreateFrame("Cooldown",nil,FR.GuideMacro,"CooldownFrameTemplate")
	...
	FR.SearchResetButton = CHAIN(CreateFrame("Button", "ZGV_GP_SearchReset", FR, "ZygorGuidesViewer_Midnight_Button"))
	...
	FR.MacroButton = CHAIN(CreateFrame("Button", nil, FR, "ZygorGuidesViewer_Midnight_Button"))
	 :SetPoint("BOTTOMRIGHT",FR,"BOTTOMRIGHT",-10,10)
	 :SetSize(80,20)
	...
	FR.BarMacroButton = CHAIN(CreateFrame("Button", nil, FR, "ZygorGuidesViewer_Midnight_Button"))
...
local function sanitize_path(p,really)
	p = p:gsub("^Zygor's ","")
	if p:find("Pets & Mounts") then really=true end
	if really then
		p = p:gsub(" Guide(s-)$",""):gsub(" Guide(s-)\\","\\")
	else
		if not p:find("Dailies") then p=p:gsub("s Guide"," Guide") end
	end
...
function GP:Refresh()
	if self.path then
		...
		FR.pathover.tip = sanitize_path(self.path,true):gsub("\\+"," |TInterface\\CHATFRAME\\ChatFrameExpandArrow:0|t ")
	...
	if self.flatgroup then
		...
		for i=1,#buts do
			...
			if g and g.name then
				-- group
				but:SetText(sanitize_path(g.name,true))
