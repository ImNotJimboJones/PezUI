...
function ZGV:QuestAutoTurnin_InGreeting()
	for qnum=1,GetNumActiveQuests() do
		for i,goal in ipairs(self.CurrentStep.goals) do
			if goal.action=="turnin" and goal.quest and goal.quest.title==GetActiveTitle(qnum) and goal:GetStatus()=="incomplete" then
...
function ZGV:QuestAutoTurnin_InGossip()
	...
	for qnum=1,GetNumGossipActiveQuests() do
		for i,goal in ipairs(self.CurrentStep.goals) do
			if goal.action=="turnin" and goal.quest and goal.quest.title==quests[qnum*3-2] and goal:GetStatus()=="incomplete" then
				self:Print("Activating quest")
