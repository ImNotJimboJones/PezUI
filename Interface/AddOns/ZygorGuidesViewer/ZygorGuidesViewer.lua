...
function ZGV:FocusStep(num,quiet)
	...
	local nextstep=self.CurrentStep:GetNextStep()
...
function ZGV:SkipStep(fast)
	...
	local step = step or self.CurrentStep:GetNextValidStep()  -- always returns a step, unless we're at the end.
...
function ZGV:TryToCompleteStep(force)
	...
	if self.pause or self.db.profile.dontprogress then
		...
		if completing then
			...
			self:Debug("Skipped goals were: "..s)
...
function ZGV:SetVisible(info,onoff)
	if onoff then self.Frame:Show() end
end
...
function ZGV:CacheCurrentMapID()
	...
	if Astrolabe.LastPlayerPosition then  -- try to avoid too much calling of SetMapToCurrentZone
		local m,f = unpack(Astrolabe.LastPlayerPosition)
		if m~=13 and m~=14 and m~=-1 and m~=485 and m~=466 and m~=613 then  -- multi-zone, whole-continent maps
	...
	--print("ZONE CHANGED, new ",self.CurrentMapID)
...
function ZGV:GoalOnClick(goalframe,button)
	...
	if button=="LeftButton" then
		...
		if goal.x and not goal.force_noway then
			...
			if WorldMapFrame:IsShown() then SetMapByID(goal.map) end
		end
...
function ZGV:GoalOnEnter(goalframe)
	...
	if goal.x and goal.y and goal.map then
		...
		local coords = math.round(goal.x)..";"..math.round(goal.y)
...
function ZGV:GoalOnLeave(goalframe,num)
	if goalframe and GameTooltip:GetOwner()==goalframe then GameTooltip:Hide() end
end
...
function ZGV:OpenQuickStepMenu(stepframe,goalframe)
	...
	if goal.map and goal.x then
		local map = GetMapNameByID(goal.map) or ('#'..goal.map)
		tinsert(menu,{
			text = L['qmenu_goal_waypoint']:format(map,goal.x,goal.y),
			tooltipTitle = L['qmenu_goal_waypoint']:format(map,goal.x,goal.y),
...
function ZGV:FindGuides(sub)
	...
	for gi,g in pairs(self.registered_groups.groups) do
		if g.name and g.name:lower():find(sub,1,true) then
			tinsert(found,g)
		end
	end
	for gi,g in pairs(self.registeredguides) do
		if (g.title_short and g.title_short:lower():find(sub,1,true))
		or (g.keywords and g.keywords:lower():find(sub,1,true)) then
			tinsert(found,g)
		end
	end
	return found
