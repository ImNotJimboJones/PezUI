...
function Addon:OnCommReceived(Prefix, Message, Distribution, Sender)
  ...
  elseif ( MessageType == "NewMacro" ) then
    ...
    for MacroData in string.gmatch(MessageData, ".-\029") do
...
function Addon:CreateMacrosFrame()
  ...
  Frame.SendToButton:SetScript("OnClick", function()
    ...
    self.EasyMenu_Send[9].disabled = (GetNumPartyMembers() == 0 or MacroIndex == 0)
...
function Addon:BuildIconTable()
  ...
  local Offset, SpellStartIndex, SpellEndIndex;
  local SpellType, SpellID, SpellTexture, FlySpellID;
...
-- print("SpellIndex: "..SpellIndex.." SpellID: "..SpellID)
      if ( SpellType ~= "FUTURESPELL" ) then
        SpellTexture = string.upper(GetSpellBookItemTexture(SpellIndex, "player"));
-- print(SpellTexture)
...
-- print("flyout!"..SpellIndex.." "..SpellID)
        ...
        if ( IsKnown and NumSlots > 0 ) then
          for FlyIndex = 1, NumSlots do
            FlySpellID, IsKnown = GetFlyoutSlotInfo(SpellID, FlyIndex);
            if ( IsKnown ) then
              SpellTexture = string.gsub(string.upper(GetSpellTexture(FlySpellID)), "INTERFACE\\ICONS\\", "");
              table.insert(self.IconTable, SpellTexture);
            end
          end
        end
