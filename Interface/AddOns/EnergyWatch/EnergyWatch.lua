...
function RegisteredEvents:UNIT_COMBO_POINTS(event, unit)
	if unit == "player" then
		EnergyWatch.UpdateSpecialPoints()
...
function RegisteredEvents:UNIT_POWER(event, unit, power)
	if unit == "player" then
		if power == "SOUL_SHARDS" or power == "HOLY_POWER" then
			EnergyWatch.UpdateSpecialPoints()
			EnergyWatch.UpdateBar()
		end
	end
...
function EnergyWatch.UpdateSpecialPoints()
	local localizedClass, englishClass = UnitClass("player");

	if EnergyWatch.powerType == 3 then
		EnergyWatch.specialPoints = GetComboPoints("player")
	elseif englishClass == "WARLOCK" then
		EnergyWatch.specialPoints = UnitPower("player", SPELL_POWER_SOUL_SHARDS)
	elseif englishClass == "PALADIN" then
		EnergyWatch.specialPoints = UnitPower("player", SPELL_POWER_HOLY_POWER)
	else
		EnergyWatch.specialPoints = nil
	end
end
...
function EnergyWatch.UpdateBar()
	...
	if EnergyWatch.specialPoints == nil then
		...
		text,_ = string.gsub(text,"&em", EnergyWatch.CapDisplayOfNumericValue(EnergyWatch.maxEnergy))
		text,_ = string.gsub(text,"&e", EnergyWatch.CapDisplayOfNumericValue(EnergyWatch.curEnergy))
		text,_ = string.gsub(text,"&c", EnergyWatch.specialPoints)
	end
