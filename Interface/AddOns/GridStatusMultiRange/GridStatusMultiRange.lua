--[[--------------------------------------------------------------------
	...
	GridStatus module for tracking multiple unit ranges.
	Created by neXter, modified by Pastamancer.
----------------------------------------------------------------------]]

local _, ns = ...
if ns.noLoad then return end
...
local GridStatusMultiRange = Grid:NewStatusModule("GridStatusMultiRange", "AceTimer-3.0")
GridStatusMultiRange.menuName = L["Range"]
...
GridStatusMultiRange.defaultDB = {
	debug = false,
	frequency = 0.5,
	-- per-status defaults are setup by RegisterStatusForRange
}

GridStatusMultiRange.extraOptions = {
	["frequency"] = {
		type = 'range', width = "double",
		name = L["Range check frequency"],
		desc = L["Seconds between range checks"],
		get = function() return GridStatusMultiRange.db.profile.frequency end,
		set = function(_, v)
				GridStatusMultiRange.db.profile.frequency = v
				GridStatusMultiRange:UpdateFrequency()
			end,
		min = 0.1,
		max = 5,
		step = 0.1,
...
function GridStatusMultiRange:EnabledStatusCount()
...
function GridStatusMultiRange:OnEnable()
	self:Grid_RangesUpdated()
	self:RegisterMessage("Grid_RangesUpdated")
	self:UpdateFrequency()
end

function GridStatusMultiRange:OnDisable()
	self:CancelTimer(self.RangeCheckTimer, true)
end

function GridStatusMultiRange:Reset()
	self:OnDisable()
	self:OnEnable()
end

function GridStatusMultiRange:EnableRange(range)
...
function GridStatusMultiRange:DisableRange(range)
...
function GridStatusMultiRange:Grid_RangesUpdated()
	ranges = {}
	ranges_status = {}
	ranges_check = {}
...
function GridStatusMultiRange:RegisterStatusForRange(range)
	...
	if not self.core:IsStatusRegistered(status_name) then
		...
		local options = {
			...
			["enable"] = {
				...
				order = 100,
				get = function()
						return GridStatusMultiRange.db.profile[status_name].enable
					end,
				set = function(_, v)
						GridStatusMultiRange.db.profile[status_name].enable = v
						if v then
							GridStatusMultiRange:EnableRange(range)
						else
							GridStatusMultiRange:DisableRange(range)
						end
					end,
			},
			["text"] = {
				...
				usage = L["<range>"],
				get = function()
						return GridStatusMultiRange.db.profile[status_name].text
					end,
				set = function(_, v)
						GridStatusMultiRange.db.profile[status_name].text = v
					end,
			},
		}

		self:RegisterStatus(status_name, status_desc, nil, false, range)

		-- override some of the default options
		local options = Grid:GetModule("GridStatus").options.args.GridStatusMultiRange.args[status_name].args

		options.enable.set = function(_, v)
			GridStatusMultiRange.db.profile[status_name].enable = v
			if v then
				GridStatusMultiRange:EnableRange(range)
			else
...
function GridStatusMultiRange:SimpleRangeCheck()
...
function GridStatusMultiRange:RangeCheck()
	for guid, unitid in GridRoster:IterateRoster() do
		local unit_range
...
function GridStatusMultiRange:UpdateFrequency()
	self:CancelTimer(self.RangeCheckTimer, true)
