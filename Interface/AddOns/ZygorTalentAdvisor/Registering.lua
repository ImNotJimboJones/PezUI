...
function me:ParseTableTalents(text,pet)
	...
	local _,class,pettype
	if pet then
		_,_,_,_,_,pettype = GetTalentTabInfo(1,false,true)
	...
	self:Debug ("Parsing type = "..tostring(pettype or class))
	...
	local spec
	local talentset = self.TalentsToNumbers[pet and pettype or class]
	if not talentset then  return nil, "ERROR: Incorrect pet/class '"..(pet and pettype or class).."' in ParseTableTalents"  end
	local chktab={}
	for i=1,#text do
