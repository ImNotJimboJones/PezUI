...
function Import:OnInitialize()
	...
	EveryQuest:RegisterModuleOptions(L["Import"], importOptions) -- Import Options Header
end
		...
		for k,v in pairs(QuestGuru_Quests[server]) do
			if (string.match(k, "^%d+$") == nil) then
				if (QuestGuru_Quests[server][k][playername]) then
					...
					if (Import:ImportQuest(questid, status, 1)) then
						count = count + 1
					end
					questid = nil
					status = nil
	...
	if (QuestHistory_List and QuestHistory_List[server] and QuestHistory_List[server][playername]) then
		for k,v in pairs(QuestHistory_List[server][playername]) do
			...
			if (questid) then
				...
				if (Import:ImportQuest(questid, status, 2)) then
					count = count + 1
				end
			end
			zone = nil
...
function Import:CheckStatus(questid)
	...
	local completed = select(7, GetQuestLogTitle(GetQuestLogIndexByID(questid)))
...
function Import:ImportQuest(questid, status, from)
	EveryQuest:Debug("We're trying to import quest "..tostring(questid).. " with a status of " .. tostring(status) .. " from addon " .. tostring(from)..".")
	quest = EveryQuest:GetQuestData(questid)
	local b = -2
	-- next lets check to see if we even need to import it, as in we may have seen it in EQ
	if (dbpc and dbpc[quest.id]) then
		-- we've seen it in EQ already, check the status
		b = dbpc[quest.id]["status"]
	end
	...
	if (cmpstatus(status, b) == 1) then
		...
		EveryQuest:AddQuest(quest.id)
		dbpc[quest.id]["imported"] = from
		dbpc[quest.id]["status"] = status
		return true
	else
		return false
	end
