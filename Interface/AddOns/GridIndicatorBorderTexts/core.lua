...
function addon:UpdateOptionsMenu()
	...
	for name, _ in pairs(indData) do
		local dt = Grid.options.args.Indicators.args[name]
		if dt ~= nil then
			local grp = indData[name].grp
			local grOpt = Grid.options.args.Indicators.args[addonId .. "_" .. grp] 
			if grOpt == nil then
				grOpt = {
					type = "group",
				...
				Grid.options.args.Indicators.args[addonId .. "_" .. grp] = grOpt
			end
			grOpt.args[name] = dt
			Grid.options.args.Indicators.args[name] = nil			
		end
	end
