local lib = LibStub:NewLibrary("X-Config", "1.4")
...
local function BuildColumn(addon, panel, groups)
	...
	for ig, g in ipairs(groups) do
		...
		group_label:SetText(LG and LG[group] or L[group] or addon.L[group])
		...
		for ie, entry in ipairs(g) do
			...
			elseif option.type == 'range' then
				...
				-- Slider
				local slider = XUI:CreateRange(row, option.text_low or L[entry..'_low'], option.text_high or L[entry..'_high'], option.min, option.max, option.step)
				label = slider.label
			...
			label:SetText(option.text or L[entry])
...
do
	...
	local function Save(panel)
		...
		for k, v in pairs(addon.config.items) do
			if v.GetValue then
				...
				if type(addon.defaults[k]) == 'boolean' then
					value = value and true or false
				end
				addon:ConfigSet(k, value)
			end
		end
	...
	local function Default(panel)
		trigger(panel.owner, 'ResetProfile')
	end

	local function ConfigSet(self, k, v)
		local k, options = k, self.config.options[k]
		if options.key then
			k = options.key
		end
		if options.subkey then
			self.opt[k][options.subkey] = v
		else
			self.opt[k] = v
		end
	end

	local function ConfigGet(self, k)
		local k, options = k, self.config.options[k]
		if options.key then
			k = options.key
		end
		if options.subkey then
			return self.opt[k][options.subkey]
		end
		return self.opt[k]
	end
	...
	function MainPanel(bootstrap)
		if not bootstrap.done then
			...
			-- Add default methods
			if not addon.ConfigSet then
				addon.ConfigSet = ConfigSet
			end
			if not addon.ConfigGet then
				addon.ConfigGet = ConfigGet
			end
